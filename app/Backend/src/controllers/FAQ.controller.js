...
export const createFAQ = asyncHandler(async(req,res,) =>{
    try{
        ...
        if(!category){
        throw new ApiError(404, "Category not Found or dosen't belongs to this store.");
        }

        const newFAQ = new FAQ({ storeId,categoryId,question,answer});
        await newFAQ.save();

        res.status(201)
        .json(
                new ApiResponse(200, newFAQ, "FAQ created Successfully.")
        );
...
export const getFAQsByCategory =asyncHandler(async(req,res) =>{
    try{
        ...
        res.status(500)
           .json(
                new ApiResponse(200, faqs,"FAQs are fetched Successfully bassed on Category")
           );
...
export const updateFAQ = asyncHandler(async(req,res) =>{
    ...
    try{
        ...
        res.status(200)
           .json(
                new ApiResponse(200,updateFAQ,"FAQ Updated Successfully")
           )
...
export const deleteFAQ = asyncHandler(async(req,res) =>{
    try{
        ...
        res.status(200)
           .json(
                new ApiResponse(200,deleteFAQ,"FAQ deleted Successfully.")
           )
...
export const getAllFAQForstore = asyncHandler(async(req,res) =>{
    try{
        ...
        res.status(200)
           .json(
                new ApiResponse(200,faqs,"ALL FAQs are fetched for store")
           )
...
export const searchFAQ = asyncHandler(async(req,res) =>{
    try{
        ...
        res.status(200)
           .json(
                new ApiResponse(200,faqs,"FAQS Searched Successfully.")
           )
...
export const getFAQAnalytics =asyncHandler(async(req,res) =>{
    try{
        ...
        res.status(200)
           .json(
                new ApiResponse(200,totalFAQs,activeFAQs,inactiveFAQs,topFAQs)
           )
    }catch(error){
        console.error("Error Searching FAQ:", error);
        throw new ApiError(500, "Something went wrong while Searching FAQ.");
